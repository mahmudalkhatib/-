import webbrowser
webbrowser.open('https://t.me/Mahmoud4090')

import os, platform, time, sys
time.sleep(0.3)
import datetime;now = datetime.date.today();target = datetime.date(2025,4,15)
if now >=target:exit("تم انتهاء تفعيل الاداة للتفعيل مره اخرى يرجى مراسله المطور: @Z_NN6")
import requests
...
@Z_NN6 ☠️
 '''
print(kkk)	
iid =input(G+'Enter Id : '+E)
tok=input(G+'Enter Token : '+E)
try:
    requests.post(f"""https://api.telegram.org/bot{token}/sendvideo?chat_id={id}&parse_mode=Markdown&video=https://t.me//m_lbj/4&caption=*- Welcome To My Tool 🐉*\n*- Wait for Checking Accounts !*\n*- The Owner :* [Махмуд](t.me/Z_NN6)""")
except Exception:
    pass

os.system('cls'if os.name=='nt'else'clear')
tlg="".join(random.choice("QWERTYUIOPASDFGHJKLZXCVBNM")for i in range(8))
kk=f'كود تسجيل الدخوت الخاص بل ادات \nCode : {tlg}'
requests.post(f"https://api.telegram.org/bot{tok}/sendMessage?chat_id={iid}&text=" + str(kk))
print(kkk)
ii= input(X+"ادخل الكود المكون من 8 احرف والذي تم ارساله اليك عبر البوت \nCODE : صورا الصيد @Z_NN6")
if ii==tlg:
	pass
else:
	print(Z+"Erorr Code");exit()
...
def info(email):
    ...
    try:
        ...
        tlg=f'''
 ⛧⋆ Instagram 𝗔𝗰𝗰𝗼𝘂𝗻𝘁 𝗙𝗼𝘂𝗻𝗱 🐝 ⋆⛧  
___________________________________
BY : @Z_NN6 • BY : @Mahmoud4090
___________________________________
⛧•|﴾𝙷𝙸𝚃 ☊ : {hit} ﴿⋆
⛧•|﴾𝙶𝙼𝙰𝙸𝙻 : {email} ﴿⋆
⛧•|﴾𝚁𝙴𝚂𝚃𝙴𝚃: {res} ﴿⋆
...
def gmail(email):
    ...
    if 'password' in response:
        info(email)
        ge+=1
        sys.stdout.write(f'''\r{F}True : {M}{ge}{Z} | False: {M}{bi}{X} | Else: {M}{be}{C} | Errur: {M}{gi}'''),sys.stdout.flush()
    else:
        be+=1
        sys.stdout.write(f'''\r{F}True : {M}{ge}{Z} | False: {M}{bi}{X} | Else: {M}{be}{C} | Errur: {M}{gi}'''),sys.stdout.flush()
...
{S}1 - {M}Check ListUser Gmail - فحص لستة جيميل
{S}2 - {M}Check ListUser Hotmail - فحص لستة هوتميل
{S}3 - {M}Random Gmail - عشوائي جيميل
{S}___________________________________
    """
   print(hh)
   try:
